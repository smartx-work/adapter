                ...
                exec (data, dataParent, index) {
                    ...
                    if (typeof newValue === 'object') {
                        if (newValue instanceof Date) {
                            if ($format) {
                                newValue = executeFormat($format, newValue)
                            }
                            if ($value) {
                    ...
                    if (Array.isArray(dataParent)) {
                        ...
                        dataParent[$key == null ? 'data' : $key] = newValue
                    }
    ...
    function walkDatas (data, dataKeys, dataIndex, dataParent) {
        ...
        if (adapter) {
            ...
            if (!data || typeof data !== 'object' || data instanceof Date) {
                ...
                const newArray = []
                data.forEach((data, i) => {
                    walkDatas(data, dataKeys, i, newArray)
                })
                newValue = newArray
            } else {
                const newObject = {}
                for (const key in data) {
                    walkDatas(data[key], dataKeys.concat(key), undefined, newObject)
                }
                newValue = newObject
            }
            ...
            if (Array.isArray(dataParent)) {
                ...
                dataParent[dataKey == null ? 'data' : dataKey] = newValue
            }
        }
    }
    ...
    function executeFormat ($format, value) {
        return $format.reduce((value, { name, args, formater }) => {
            if (name === null) {
                ...
                return formats[name](value, ...args)
            }
        }, value)
    }
