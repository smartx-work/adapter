...
function adapter (setting) {
    ...
    return {
        input (data) {
            return _innerInput(data, undefined, undefined, data)
        },
        _innerInput,
    }

    function _innerInput (data, dataIndex, row, root) {
        const dataParent = { data }
        runtimeRootData = root
        walkDatas(data, [], dataIndex, dataParent, row)
        ...
        const rules = (Array.isArray(setting) ? setting : [setting]).map((setting, index) => {
            ...
            if ($format) {
                ...
                newFormats.forEach(format => {
                    ...
                    if (!formater) {
                        formater = format.formater = formats[name]
                    }

                    // 初始化修剪自定义参数的变量，让runtime保持最高优先级
                    let defineArgs = String(formater).match(/\(([^)]*)\)/)
                    if (defineArgs === null) { // 不带括号的箭头函数：value => value
                        defineArgs = []
                    } else {
                        defineArgs = defineArgs[1].replace(/\s/g, '').replace(/,$/, '')
                        defineArgs = defineArgs ? defineArgs.split(',') : []
                    }

                    let useRuntime = false
                    let customArgsLength = 0
                    for (let i = 1; i < defineArgs.length; i++) {
                        if (defineArgs[i] === 'runtime') {
                            useRuntime = true
                            break
                        }
                        customArgsLength++
                    ...
                    format.useRuntime = useRuntime
                    format.customArgsLength = customArgsLength
                })
            ...
            if ($increase) {
                ...
                if ($increase instanceof Array) {
                    ...
                    for (const key in $increase) {
                        const cmd = $increase[key]

                        // 扩展$increase指令，添加支持直接量和函数返回值
                        ...
                        } else if (cmd && cmdType === 'object' && !(cmd instanceof Date || cmd instanceof RegExp)) {
                            adapters.push({
                                key,
                                ...
                                if (Array.isArray(newValue)) {
                                    ...
                                    if ($increase) {
                                        $increase.forEach(item => {
                                            ...
                                            newObject[key] = adapter._innerInput(newValue, index, row, runtimeRootData)
                                        })
                                    }
                                    ...
                                    if ($reduce) {
                                        $reduce.forEach(item => {
                                            const { key, adapter } = item
                                            Object.assign(newObject, adapter._innerInput(newValue[key], index, newValue, runtimeRootData))
                                        })
                                    }
    ...
    function executeFormat ($format, value, runtime) {
        return $format.reduce((value, { args, formater, useRuntime, customArgsLength }) => {
            // 自定义的参数
            const customArgs = args ? [...args] : []
            if (useRuntime) {
                customArgs.length = customArgsLength
                customArgs.push(runtime)
            }

            return formater(value, ...customArgs)
        }, value)
    }
...
// $clears 声明清理规则，设为true是，则使用[null, undefined, '']对数据进行过滤处理

// 指令集
// $key:newKey 拷贝一个新的字段，假如缺省newKey，则以同名字段拷贝
// $type:[Number,Boolean,Date,String,Array] 对数据进行类型转换，规则符合JS内置类型转换
// $default:[data,function] 设置null或undefined类型数据的默认值，假如是函数，则使用函数返回值，一旦定义了`$default`指令，那么符合`$default`指令的数据不在执行其他指令
// $enum:[array] 枚举类型转换
// $emap:[object] 映射转换
